...
export default function SpeakerListCard({ speaker, status, onToggle }: Props) {
  return (
    <div className="bg-white rounded-2xl border border-gray-100 shadow-sm px-4 py-3 flex items-center gap-4 hover:shadow-md transition-shadow">
      {/* Avatar */}
      {speaker.photoUrl ? (
        <img
        ...
        <div className="w-12 h-12 rounded-full bg-gradient-to-br from-indigo-400 to-purple-500 flex items-center justify-center text-white font-semibold text-base flex-shrink-0">
      ...
      <div className="flex-1 min-w-0">
        ...
        <p className="text-xs text-gray-400 truncate mt-0.5 italic">{speaker.topic}</p>
      ...
      {/* Status toggles as compact pills */}
      <div className="flex flex-col gap-1.5 flex-shrink-0">
        ...
        >
          {status.inRoom ? "✓ In room" : "Not in room"}
        </button>
      </div>
    </div>
  );
