...
export default function SpeakerCard({ speaker, status, onToggle }: Props) {
    ...
    <div className="bg-white rounded-2xl shadow-sm border border-gray-100 p-5 flex flex-col gap-4 hover:shadow-md transition-shadow">
      ...
      <div className="flex items-start justify-between gap-3">
        <div className="flex items-center gap-3">
          {speaker.photoUrl ? (
            ...
            <div className="w-12 h-12 rounded-full bg-gradient-to-br from-indigo-400 to-purple-500 flex items-center justify-center text-white font-semibold text-lg flex-shrink-0">
              {speaker.name.split(" ").map(n => n[0]).slice(0, 2).join("")}
            </div>
          )}
          <div>
            <h3 className="font-semibold text-gray-900 text-base leading-tight">{speaker.name}</h3>
            <p className="text-sm text-gray-500 mt-0.5">{speaker.topic}</p>
          </div>
        </div>
      ...
      <div className="flex items-center gap-2 flex-wrap">
        <div className="flex items-center gap-1.5 bg-gray-50 rounded-lg px-3 py-1.5">
          ...
          <span className="text-sm font-medium text-gray-700">{speaker.timeStart} – {speaker.timeEnd}</span>
        </div>
        <span className={`text-xs font-medium px-3 py-1.5 rounded-lg ${getRoomColor(speaker.room)}`}>
          {speaker.room}
        </span>
      </div>
      ...
      <div className="flex flex-col gap-2 pt-1 border-t border-gray-50">
        <label className="flex items-center gap-3 cursor-pointer group">
          ...
          <span className="text-sm text-gray-700">Arrived at location</span>
        </label>
        ...
        <label className="flex items-center gap-3 cursor-pointer group">
          ...
          <span className="text-sm text-gray-700">In room — ready to present</span>
        </label>
      </div>
    </div>
  );
